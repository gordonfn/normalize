...
module.exports = (data, coord) => {
  ...
  if (data.ActivityEndDate) {
    ...
    if (isValidDate(`${data.ActivityEndDate}T${data.ActivityEndTime}`)) {
      ActivityEndTimestamp = formatISO(
        toDate(`${data.ActivityEndDate}T${data.ActivityEndTime}`, dateOptions),
        dateOptions
      )
  ...
  if (data.AnalysisStartDate) {
    ...
    data.AnalysisStartTimeZone = formatTimeZone(data.AnalysisStartTimeZone)
    if (
      isValidDate(`${data.AnalysisStartDate}T${data.AnalysisStartTime}${data.AnalysisStartTimeZone || 'Z'}`)
    ) {
      AnalysisStartTimestamp = `${data.AnalysisStartDate}T${data.AnalysisStartTime}${data.AnalysisStartTimeZone}`
      AnalysisStartTimeZone = data.AnalysisStartTimeZone
    } else {
      console.error(
        'AnalysisStartTimestamp Invalid',
        `${data.AnalysisStartDate}T${data.AnalysisStartTime}${
          data.AnalysisStartTimeZone || 'Z'
        }`,
        y,
        x
      )
