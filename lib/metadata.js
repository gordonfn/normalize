const defaultMetadata = {
  id: null,
  //observations: 0,    // deprecated - sent in by client
...
const parseExtents = (obj) => {
  ...
  newObj.spatial_extent = obj.spatial_extent
    ? obj.spatial_extent
        ...
        .split(',')
        .map((v, idx) =>
          v ? Number.parseFloat(v) : defaultMetadata.spatial_extent[idx]
        )
  ...
  newObj.vertical_extent = obj.vertical_extent
    ? obj.vertical_extent
        ...
        .split(',')
        .map((v, idx) =>
          v ? Number.parseFloat(v) : defaultMetadata.vertical_extent[idx]
        )
  ...
  newObj.temporal_extent = obj.temporal_extent
    ? obj.temporal_extent
        ...
        .split(',')
        .map((v, idx) =>
          v ? new Date(v) : defaultMetadata.temporal_extent[idx]
        )
...
const update = (data) => {
  ...
  if ((data.activity_start_date || null) !== null) {
    const minTime = new Date(
      `${data.activity_start_date}T${data.activity_start_time || '00:00:00'}${
        data.activity_start_time_zone || 'Z'
      }`
    )
    const maxTime =
      (data.activity_end_date || null) !== null
        ? new Date(
            Math.max(
              minTime,
              new Date(
                `${data.activity_end_date}T${
                  data.activity_end_time || '00:00:00'
                }${data.activity_end_time_zone || 'Z'}`
              )
            )
          )
        : minTime
    if (
      metadata.temporal_extent[0] === -Infinity &&
      metadata.temporal_extent[1] === Infinity
    ) {
  ...
  if (typeof data.activity_depth_height_measure_normalized === 'number') {
    if (
      metadata.vertical_extent[0] === -Infinity &&
      metadata.vertical_extent[1] === Infinity
    ) {
