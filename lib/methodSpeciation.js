...
module.exports = (characteristic, methodSpeciation, measure) => {
  if (
    (typeof measure !== 'number' || !isFinite(measure))
    || (!methodSpeciation || methodSpeciation === 'Unspecified')
  ...
  const toMolarMass = molarMass(normalizedMethodSpeciation.replace(/^as/, '').trim())
